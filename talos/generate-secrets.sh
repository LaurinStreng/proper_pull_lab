...
encryptedPassword=$(kubectl --kubeconfig ./kubeconfig create secret generic script --dry-run=client --from-literal "script=$password" -o json | kubeseal --kubeconfig ./kubeconfig -o json | jq -r '.spec.encryptedData.script')
